...
Счётчик инициализируется в SPA-режиме с `defer: true`. Первый просмотр и
каждый переход между страницами Next.js отправляются отдельным методом `hit`,
поэтому внутренние переходы учитываются без дубля стартовой загрузки.
Вебвизор и чтение ecommerce-событий из `dataLayer` включаются только после
явного согласия пользователя на аналитику.
