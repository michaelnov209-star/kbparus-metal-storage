...
GitHub workflow `Post-deploy Smoke` автоматически проверяет только Production.
Защищённые Vercel Preview пропускаются намеренно: bypass-секрет нельзя передавать
коду из непроверенной ветки. Preview проверяется вручную после авторизации либо
через `workflow_dispatch` для доверенного deployment.
