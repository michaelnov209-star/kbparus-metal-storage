...
SMTP_PASSWORD=
SMTP_FROM=
SMTP_FROM_NAME=КБ Парус
```

`SMTP_FROM` можно не задавать, тогда используется `SMTP_USER`.

Один и тот же SMTP-транспорт используется для доставки заявок и системных
писем Payload (например, восстановления пароля). Адаптер включается только при
полном наборе SMTP-переменных; тестовый Ethereal-аккаунт в production не
создаётся.
