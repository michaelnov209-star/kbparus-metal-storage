...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    UPDATE "calculator_profiles_options" AS option
    SET "option_id" = expected.option_id
    FROM "calculator_profiles" AS profile, expected_options AS expected
    WHERE option."_parent_id" = profile."id"
      AND profile."slug"::text = expected.parent_slug
      AND option."_order" = expected.option_order
      AND option."title" = expected.option_title
      AND option."option_id" IS DISTINCT FROM expected.option_id
  `);
...
export async function down(_args: MigrateDownArgs): Promise<void> {
  throw new Error(
    "Calculator option business-key repair is irreversible. Restore a verified Neon snapshot instead."
  );
}