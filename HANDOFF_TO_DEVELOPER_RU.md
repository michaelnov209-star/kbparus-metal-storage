...
- [ ] `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` — для уведомлений менеджеров
- [ ] Реальные фотографии оборудования (заменить Pexels в `data/storageSystems/visualAssets.ts`)
- [ ] Описания и характеристики продуктов для 14 категорий (без продуктов)
...
- [ ] Rate-limiting в памяти (Lambda холодным стартом сбрасывает счётчик) — для прода желателен Upstash Redis
- [ ] CSP не настроен (требует рефакторинга inline-стилей в `LinePageStyles.tsx`)
- [ ] 32 calculator-профиля из нового Excel ещё не реализованы (только 6 базовых)
...
3. Подключить Yandex Metrika / GA4 для аналитики (1 час)
4. UTM-трекинг (3 часа)
5. Реальные фото оборудования (от заказчика)

### Что ИСПРАВЛЕНО в сессии 2 от 2026-05-09
- ✅ Цена `weight-scale` 3 ₽ → 90 000 ₽ (из Excel)
- ✅ Security headers (HSTS, XFO, XCTO, RP, PP)
- ✅ Rate-limiting на /api/leads (5 req/min/IP)
- ✅ Honeypot + speed-trap в форме заявок
- ✅ Серверная валидация телефона/email
- ✅ Anti-double-submit в LeadForm
- ✅ Обновлены npm-пакеты до последних patch-версий
- ✅ +14 продуктов в каталоге (категории 2 и 4)
- ✅ Заменён баннер линий окраски
...
| 2026-05-09 (утро) | Claude Sonnet 4.6 | SEO-инфраструктура, CSS-фиксы, удаление долга, документация |
| 2026-05-09 (день) | Claude Sonnet 4.6 | Обновление npm-пакетов, +14 продуктов (cat 2, 4), новый баннер, security headers, rate-limit, honeypot, валидация форм |
