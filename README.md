...
- `/catalog/[id]/[productId]` поддерживает два режима товара:
  - **обычный товар** — карточка, галерея и форма заявки; характеристики и состав подбора встроены в первый экран товара рядом с галереей;
  - **товар с калькулятором** — единый `Calculator`, ограниченный одним профилем через `calculatorProfileId`; выбор модели пропускается, пользователь сразу видит параметры, затем результат; характеристики и состав подбора встроены в первый экран товара, общая форма не дублируется.
...
- Первый экран и контентные карточки используют viewport-анимацию на `IntersectionObserver`: на мобильных эффекты короче, а при `prefers-reduced-motion` полностью отключаются. Калькулятор как рабочий инструмент всегда отображается сразу и не зависит от прокрутки.
