...
- `/catalog/[id]/[productId]` поддерживает два режима товара:
  ...
  - **товар с калькулятором** — полный трёхшаговый `Calculator`, ограниченный одним профилем через `calculatorProfileId`; характеристики, состав подбора и документы расположены ниже калькулятора, общая форма не дублируется.
