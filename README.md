...
- Каталог главной вынесен в компонент `components/CatalogGrid.tsx`: карточки стали более визуальными, с крупными фото, минимальным текстом и просмотром изображения в большом lightbox по клику.
...
- Визуальный принцип “продукт без вставленного квадратного фото” применен к каталогу, ассортименту внутри категории, карточке товара и превью калькулятора через общий фон, blur/backdrop и крупные product-visual зоны.
...
- Каталог оборудования из 17 разделов, строго по текущей структуре Excel-листа `Главная`, без публичных ссылок на ячейки; карточки открывают категорию, а фото можно раскрыть крупно для просмотра.
